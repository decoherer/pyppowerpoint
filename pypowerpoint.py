...
if __name__ == '__main__':
    def test():
        ...
        subtitle.text = "python-pptx was here!"
        prs.save('test.pptx')
    def advrtest(): # default slide size = 10" x 7.5"
        prs = Presentation()
        # addtestslide(prs)
        addadvrslide(prs,'Coincidence Histogram','Coincidence count rate vs delay time','h12.png')
        folder = "."
        # files = glob.glob(folder+"\*.png")
        # for f in files:
        #     name = os.path.splitext(os.path.split(f)[1])[0]
        #     name = name.replace('mfd','MFD').replace('shg','SHG').replace('ape','APE').replace('rpe','RPE').replace('bragg','Bragg')
        #     addadvrslide(prs,name[:1].capitalize()+name[1:],'',f)
        # tasklist /V /FI "WindowTitle eq test.pptx*"
        os.system('taskkill /F /FI "WINDOWTITLE eq test.pptx*" ')
        # prs.save('test.pptx')
        # os.startfile('C:\\py\\advr\\test.pptx')
        prs.save(folder+'\out.pptx')
        os.startfile(folder+'\out.pptx')
